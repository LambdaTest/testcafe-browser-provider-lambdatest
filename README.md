...
```bash
git clone https://github.com/LambdaTest/testcafe-browser-provider-lambdatest && cd testcafe-browser-provider-lambdatest
...
- `LT_BUILD` - Build name on TestMu AI.
- `LT_CAPABILITY_PATH` - Path to a file which contains additional capability options as a JSON file.
- `LT_RESOLUTION` - Sets the screen resolution for desktop browsers in `${width}x${height}` format.
...
Run tests for a single browser configuration:
